...
const useWebSocketClient = (HEADER_TOKEN: {
  ...
  const mapMembers = (arr: any[]) => {
    return arr.map(
      (member: { memberId: number; nickName: string; imageLink: string }) => {
        return { nickName: member.nickName, imageLink: member.imageLink };
      }
    );
  };
  ...
  useEffect(() => {
    ...
    const defaultChatSetting = async () => {
      await axios.get(`https://ngether.site/chat/room/enter/${roomId}`, {
        headers: HEADER_TOKEN,
      });
      await axios
        .get(`https://ngether.site/chat/room/messages/${roomId}`, {
          headers: HEADER_TOKEN,
        })
            ...
            ws.subscribe(
              ...
              async (messages) => {
                ...
                if (
                  ...
                  alert(`${sharingData.title} 채팅방에서 퇴장당하셨습니다.`);
                  push('/');
                }
                setMessages((prevMessages) => [
                  ...prevMessages,
                  transDateFormChatMessage(message),
                ]);
              },
              HEADER_TOKEN
            );
    ...
    const checkChatMember = () => {
      axios
        .get(`https://ngether.site/chat/room/${roomId}/memberList`)
        .then((res) => {
          const members = mapMembers(res.data);
...
export const transDateFormChatMessage = (chatMessage: chatMessageType) => {
  return {
    ...chatMessage,
    createDate: transDateTimeFormat(chatMessage.createDate),
  };
};
