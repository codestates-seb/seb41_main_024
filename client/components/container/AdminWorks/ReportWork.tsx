...
const ReportWork = () => {
  ...
  const { data, isSuccess, refetch } = useQuery(
    ['reports', pageInfo.page],
    () => getReport(pageInfo.page),
    {
  ...
  return (
    <div className="flex flex-col text-center">
      ...
      {isSuccess && reports && (
        <>
          <p className="my-[16px] text-xs">
            게시물은 해당 게시글로 이동하여 처리하실 수 있습니다.
          </p>
                    ...
                    <Accordion>
                      ...
                      >
                        <Typography className="text-left">
                          <span className="text-gray-400">{`신고된 ${
                            (report.reportType === 'board' && '게시글') ||
                            '채팅방'
                          }`}</span>
                      ...
                      <AccordionDetails>
                        {report.reportType === 'board' && (
                          <ReportBoardDetail
                            handleNavigate={() =>
                              handleNavigate(report.reportedId, report.reportId)
                            }
                            handleDeleteReport={() =>
