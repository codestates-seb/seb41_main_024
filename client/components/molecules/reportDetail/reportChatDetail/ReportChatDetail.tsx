...
const ReportChatDetail = ({
  ...
  handleBlockUser,
  handleDeleteReport,
}: reportChatDetailType) => {
  const [chatDataSet, setChatDataSet] = useState<ChatDataSetType>({
    chatLog: [],
    chatMembers: [],
  });
  ...
  return (
    <Fragment>
      <div className="flex">
        <Button className="flex-1" onClick={handleRenderChatLog}>
          조회하기
        </Button>
        <Button className="w-[100px]" onClick={handleDeleteReport}>
          신고 처리완료
        </Button>
        <Button className="w-[100px]" onClick={handleDeleteReport}>
          신고 반려
        ...
        <div>
          ...
          <ul className="flex flex-col mt-2 max-h-[400px] overflow-auto">
            {chatDataSet.chatLog.map((log: ChatLogType) => (
              <li
                className="text-left text-s mt-3 mb-2 border-b-2"
                key={log.chatMessageId}
              >
                <div>
                  <p>
